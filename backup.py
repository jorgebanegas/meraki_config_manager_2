...
def backup(network_id):
    ...
    print("Backing Up Camera Configurations")
    try:
        temp_cameras = {}
        temp1 = []
        devices = dashboard.networks.getNetworkDevices(network_id)
        for device in devices:
            if device["model"].startswith("MV"):
                temp1.append(dashboard.camera.getDeviceCameraQualityAndRetention(device["serial"]))
                temp1.append(dashboard.camera.getDeviceCameraSense(device["serial"]))
                temp1.append(dashboard.camera.getDeviceCameraVideoSettings(device["serial"]))
                temp1.append(dashboard.camera.getDeviceCameraVideoLink(device["serial"]))

                temp_cameras[device["serial"]] = temp1
        data['cameras'] = temp_cameras
    except:
        print("MS is not Supported")
